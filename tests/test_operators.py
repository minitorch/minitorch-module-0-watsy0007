...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    s_a = sigmoid(a)
    # property 1: range check
    assert 0.0 <= s_a <= 1.0, f"Sigmod {s_a} not between 0.0 and 1.0"

    s_neg_a = sigmoid(-a)

    # property 2: symetry check
    assert_close(1 - s_a, s_neg_a)

    # property 3: center point check
    assert_close(sigmoid(0.0), 0.5)

    # property 4: strictly increasing check
    epsilon = 1e-7
    if -20 <= a <= 20:
        assert sigmoid(a + epsilon) > s_a, "sigmoid not strictly increasing"
